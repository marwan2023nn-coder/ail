...
export class ServerInfo {
    ...
    fetchRemoteInfo = async () => {
        // Parallelize network requests to improve startup/refresh performance
        await Promise.all([
            this.fetchConfigData(),
            this.getRemoteInfo<Array<{ id: string; version: string }>>(
                this.onGetPlugins,
                parseURL(`${this.server.url}/api/v4/plugins/webapp`),
            ),
            this.getRemoteInfo<{ SkuShortName: string }>(
                this.onGetLicense,
