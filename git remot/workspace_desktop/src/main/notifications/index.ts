...
class NotificationManager {
                    ...
                    if (!failed) {
                        ...
                        if (mainWindow) {
                            ...
                            if (isBuzz) {
                                ...
                                Promise.all([
                                    MainWindow.sendToRenderer(PLAY_SOUND, 'Buzz'),
                                    this.animateWindow(),
                                ]).then(() => {
                                    setTimeout(() => {
                                    ...
                                    log.debug('Faild to Run Buzz sound', serverName, error);
                                });
                            }
                        }
    ...
    private async animateWindow() {
        ...
        for (let i = 0; i < shakeCount; i++) {
            const offsetX = i % 2 === 0 ? shakeIntensity : -shakeIntensity;
            const offsetY = i % 4 < 2 ? shakeIntensity : -shakeIntensity;
...
export async function getDoNotDisturb() {
    ...
    if (process.platform === 'darwin' && !isDev) {
        try {
            ...
            const {getDoNotDisturb: getDarwinDoNotDisturb} = require('macos-notification-state');
