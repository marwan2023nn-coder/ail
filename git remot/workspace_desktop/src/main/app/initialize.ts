...
import {CALLS_PLUGIN_ID} from 'common/utils/constants';
...
export const mainProtocol = electronBuilder.protocols?.[0]?.schemes?.[0];
...
function initializeBeforeAppReady() {
    ...
    protocol.registerSchemesAsPrivileged([
        {scheme: 'workspace-desktop', privileges: {standard: true}},
    ]);
}
...
async function initializeAfterAppReady() {
    protocol.handle('workspace-desktop', (request: Request) => {
    ...
    app.setAppUserModelId('Workspace.Desktop'); // Use explicit AppUserModelID
    const defaultSession = session.defaultSession;
    defaultSession.webRequest.onHeadersReceived((details, callback) => {
        const url = parseURL(details.url);
        if (url?.protocol === 'workspace-desktop:' && url?.pathname.endsWith('html')) {
            callback({
                responseHeaders: {
                    ...details.responseHeaders,
                    'Content-Security-Policy': [`default-src 'self'; style-src 'self' 'nonce-${NonceManager.create(details.url)}'; media-src data:; img-src 'self' data:; font-src 'self' data: https://res-1.cdn.office.net; connect-src 'self' wss: ws:;`],
                },
            });
            return;
        }

        if (url?.pathname.includes(CALLS_PLUGIN_ID) && (url?.pathname.includes('widget.html') || url?.pathname.includes('expanded'))) {
            callback({
                responseHeaders: {
                    ...details.responseHeaders,
                    'Content-Security-Policy': ["default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data: https://res-1.cdn.office.net; connect-src 'self' wss: ws:;"],
                },
            });
            return;
        }
    ...
    defaultSession.webRequest.onBeforeSendHeaders((details, callback) => {
        try {
            ...
            const isApiV4 = details.url.includes('/api/v4/');
            const isCallsWidget = details.url.includes(CALLS_PLUGIN_ID);
            const headers = {...details.requestHeaders};

            // Add custom token for API v4 requests and calls widget requests to known servers
            if (server && (isApiV4 || isCallsWidget)) {
                if (!('X-Custom-Token' in headers)) {
                    headers['X-Custom-Token'] = 'MY_SECRET_KEY';
                }
            }

            if (server && server.preAuthSecret && !('X-Mattermost-Preauth-Secret' in headers)) {
                headers['X-Mattermost-Preauth-Secret'] = server.preAuthSecret;
            }
