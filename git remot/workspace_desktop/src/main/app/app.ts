...
export function handleAppBeforeQuit() {
    ...
    // Force config and state save on quit to prevent corruption/loss
    Config.saveSync();

    // Use dynamic require to avoid circular dependencies and test failures
    try {
        // eslint-disable-next-line global-require
        const AppVersionManager = require('../AppVersionManager').default;
        AppVersionManager.saveSync();
    } catch (e) {
        // ignore
    }
    ...
    try {
        // eslint-disable-next-line global-require
        const DownloadsManager = require('../downloadsManager').default;
        DownloadsManager.saveSync();
    } catch (e) {
        // ignore
    }
