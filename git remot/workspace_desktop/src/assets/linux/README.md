...
Otherwise, first locate the extracted directory in your desired directory (e.g. `/opt/workspace-desktop`) and follow the steps below. (This directory also contains this README here.)

### Desktop launcher

Execute the script file to create a `Workspace.Desktop` file.

```
/opt/workspace-desktop/create_desktop_file.sh
```
...
```
mv Workspace.Desktop ~/.local/share/applications/
```
...
```sh
# assuming that /opt/workspace-desktop/workspace-desktop is the executable file.
export PATH=$PATH:/opt/workspace-desktop
```

Alternatively, you can also create a symbolic link for the application.

```sh
sudo ln -s /opt/workspace-desktop/workspace-desktop /usr/local/bin/
```
