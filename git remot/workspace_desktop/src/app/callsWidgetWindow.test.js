...
import {BrowserWindow, desktopCapturer, systemPreferences, ipcMain, dialog} from 'electron';
...
jest.mock('electron', () => ({
    app: {
        ...
        getPath: jest.fn().mockReturnValue('/path/to/downloads'),
    },
...
describe('main/windows/callsWidgetWindow', () => {
    ...
    describe('handleRemoteControlRequestPermission', () => {
        const view = {
            id: 'main-view',
            webContentsId: 1,
        ...
        beforeEach(() => {
            ...
            PermissionsManager.doPermissionRequest.mockResolvedValue(true);
            MainWindow.get.mockReturnValue({webContents: {id: 100}});
        });

        afterEach(() => {
            jest.clearAllMocks();
            delete callsWidgetWindow.remoteControlAllowedForSession;
        });

        it('should return true if permission is granted and user approves', async () => {
            const result = await callsWidgetWindow.handleRemoteControlRequestPermission({sender: {id: 1}});
            expect(PermissionsManager.doPermissionRequest).toHaveBeenCalledWith(1, 'remoteControl', expect.anything());
            expect(dialog.showMessageBox).toHaveBeenCalled();
            expect(result).toBe(true);
            expect(callsWidgetWindow.remoteControlAllowedForSession).toBe(true);
        });

        it('should return true immediately if already allowed for session', async () => {
            callsWidgetWindow.remoteControlAllowedForSession = true;
            const result = await callsWidgetWindow.handleRemoteControlRequestPermission({sender: {id: 1}});
            expect(PermissionsManager.doPermissionRequest).toHaveBeenCalled();
            expect(dialog.showMessageBox).not.toHaveBeenCalled();
            expect(result).toBe(true);
        });

        it('should return false if global permission is denied', async () => {
            PermissionsManager.doPermissionRequest.mockResolvedValue(false);
            const result = await callsWidgetWindow.handleRemoteControlRequestPermission({sender: {id: 1}});
            expect(result).toBe(false);
            expect(dialog.showMessageBox).not.toHaveBeenCalled();
        });

        it('should return false if user denies session approval', async () => {
            dialog.showMessageBox.mockResolvedValue({response: 0});
            const result = await callsWidgetWindow.handleRemoteControlRequestPermission({sender: {id: 1}});
            expect(result).toBe(false);
            expect(callsWidgetWindow.remoteControlAllowedForSession).not.toBe(true);
        });
    });

    describe('handleSendRemoteControlEvent', () => {
        const view = {
            id: 'main-view',
            webContentsId: 1,
            isDestroyed: jest.fn().mockReturnValue(false),
        ...
        const callsWidgetWindow = new CallsWidgetWindow();
        callsWidgetWindow.mainView = view;
        callsWidgetWindow.checkAccessibilityPermissions = jest.fn().mockReturnValue(true);
        ...
        it('should handle remote events correctly when allowed', async () => {
            callsWidgetWindow.remoteControlAllowedForSession = true;
            const remoteEvent = {type: 'mousemove', x: 0.5, y: 0.2};
            await callsWidgetWindow.handleSendRemoteControlEvent({sender: {id: 1}}, remoteEvent);

            // Since we use native libraries that are hard to mock in unit tests easily,
            // we primarily check that it doesn't throw and respects the sender ID.
        });

        it('should block events when not allowed for session', async () => {
            callsWidgetWindow.remoteControlAllowedForSession = false;
            const remoteEvent = {type: 'mousemove', x: 0.5, y: 0.2};
            await callsWidgetWindow.handleSendRemoteControlEvent({sender: {id: 1}}, remoteEvent);
        });

        it('should allow event from widget window when allowed', async () => {
            callsWidgetWindow.remoteControlAllowedForSession = true;
            const remoteEvent = {type: 'mousemove', x: 0.1, y: 0.1};
            callsWidgetWindow.win = {webContents: {id: 3}};
            await callsWidgetWindow.handleSendRemoteControlEvent({sender: {id: 3}}, remoteEvent);
        });

        it('should block event from wrong webContentsId', async () => {
            callsWidgetWindow.remoteControlAllowedForSession = true;
            const remoteEvent = {type: 'mousemove', x: 0.1, y: 0.1};
            callsWidgetWindow.win = {webContents: {id: 3}};
            await callsWidgetWindow.handleSendRemoteControlEvent({sender: {id: 4}}, remoteEvent);
        });
    });
    ...
    describe('handleCreateCallsWidgetWindow', () => {
        ...
        it('should switching calls', async () => {
            let func;
            ipcMain.on.mockImplementation((_, callback) => {
                func = callback;
            });
            ...
            const window = {webContents: {id: 3}, isDestroyed: jest.fn().mockReturnValue(false)};
            callsWidgetWindow.win = window;
            callsWidgetWindow.mainView = {
                view: {server: {id: 'server-1'}},
                webContentsId: 1,
                isDestroyed: jest.fn().mockReturnValue(false),
                sendToRenderer: jest.fn(),
            };
        ...
        it('should switching calls between servers and existing call', async () => {
            let func;
            ipcMain.on.mockImplementation((_, callback) => {
                func = callback;
            });
