...
type ExtendedDesktopAPI = DesktopAPI & {
    ...
    requestRemoteControlPermission: () => Promise<any>;
    terminateRemoteControlSession: () => void;
    shareScreen: (sourceID: string, withAudio: boolean, screenID?: string) => void;
};

(window as any).screenSharingSessionIDForCurrentCall = '';
...
const desktopAPI: ExtendedDesktopAPI = {
    ...
    getStunURI: () => ipcRenderer.invoke(GET_CONFIGURATION).then((config) => config.stunURI),
    ...
    shareScreen: (sourceID: string, withAudio: boolean, screenID?: string) => ipcRenderer.send(CALLS_WIDGET_SHARE_SCREEN, sourceID, withAudio, screenID),
    ...
    sendRemoteControlEvent: (event) => ipcRenderer.send(CALLS_SEND_REMOTE_CONTROL_EVENT, event),
    requestRemoteControlPermission: () => ipcRenderer.invoke(CALLS_REMOTE_CONTROL_REQUEST_PERMISSION),
    terminateRemoteControlSession: () => ipcRenderer.send(CALLS_REMOTE_CONTROL_TERMINATE_SESSION),
