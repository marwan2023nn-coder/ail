...
describe('common/config', () => {
    ...
    describe('set', () => {
        ...
        it('should allow servers to be set using this method', () => {
            const config = new Config();
            config.reload = jest.fn();
            config.init(configPath, appName, appPath);
            ...
            expect(config.localConfigData.servers).toContainEqual({...buildServer, name: 'build-team-2'});
        });
    });
    ...
    describe('saveLocalConfigData', () => {
        ...
        it('should emit error when writeFile throws an error', async () => {
            const config = new Config();
            config.reload = jest.fn();
            config.init(configPath, appName, appPath);
            config.localConfigData = {test: 'test'};
            config.combinedData = {...config.localConfigData};
            config.writeFile = jest.fn().mockImplementation((path, data, callback) => {
    ...
    describe('loadLocalConfigFile', () => {
        ...
        it('should use defaults and attempt to recover if validation fails', () => {
            const config = new Config();
            config.reload = jest.fn();
            config.init(configPath, appName, appPath);
            config.defaultConfigData = {test: 'test'};
            ...
            const configData = config.loadLocalConfigFile();
            expect(configData).toStrictEqual({test: 'test', version: -1});
        });
