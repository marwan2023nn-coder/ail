...
const BASE_REGISTRY_KEY_PATH = 'SOFTWARE\\Policies\\Workspace';
...
export default class RegistryConfig extends EventEmitter {
   ...
   */
    private getRegistryEntry(key: string, name?: string) {
        ...
        const {HKEY} = require('registry-js');
   ...
   */
    private getRegistryEntryValues(hive: any, key: string, name?: string) {
        try {
            // eslint-disable-next-line global-require
            const {enumerateValues} = require('registry-js');
            const results = enumerateValues(hive, key);
            if (!results || results.length === 0) {
                return undefined;
            }
            if (name) { // looking for a single entry value
                return results.find((item: RegistryValue) => item.name === name)?.data;
            }
