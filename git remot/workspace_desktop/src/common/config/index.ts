...
export class Config extends EventEmitter {
    ...
    /**
     * Triggers a synchronous save of the current configuration data.
     * Use this only when absolutely necessary, such as during application shutdown.
     */
    saveSync = (): void => {
        ...
        log.verbose('Saving config data to file synchronously...');

        try {
            this.writeFile(this.configFilePath, this.localConfigData);
        } catch (error) {
            log.error('Failed to save config data synchronously:', error);
        }
    };
    ...
    private saveLocalConfigData = (): void => {
        ...
        if (this.saving) {
            this.saving = this.saving.then(writePromise);
        } else {
            this.saving = writePromise();
        }
    };
    ...
    private loadLocalConfigFile = (): AnyConfig => {
        ...
        try {
            if (!fs.existsSync(this.configFilePath)) {
                log.info(`Configuration file ${this.configFilePath} not found. Creating with defaults.`);
            ...
            try {
                const validatedConfigData = Validator.validateConfigData(configData);
                if (validatedConfigData) {
                    configData = validatedConfigData;
                    log.info(`Successfully loaded configuration file from ${this.configFilePath}`);
                } else {
                    log.error(`Provided configuration file ${this.configFilePath} does not validate. Attempting to recover.`);

                    // If it doesn't validate, we try to use it anyway but merge with defaults for safety
                    configData = Object.assign({}, copy(this.defaultConfigData), configData);
                }
            } catch (validationError) {
                log.error(`Configuration validation failed for ${this.configFilePath}. Attempting to recover. Error: ${validationError}`);

                // If validation fails, we try to use it anyway but merge with defaults for safety
                configData = Object.assign({}, copy(this.defaultConfigData), configData);
            }
        } catch (e) {
            log.error(`Failed to load configuration file from ${this.configFilePath}. Using defaults to ensure application stability. Error: ${e}`);
            configData = copy(this.defaultConfigData);

            // Only overwrite if it's a JSON parse error or file is missing, otherwise we might be overwriting a good config that just failed a minor validation
            if (e instanceof SyntaxError || (e as any).code === 'ENOENT') {
                this.writeFile(this.configFilePath, configData);
            }
        }
        return configData;
    ...
    private writeFile = (filePath: string, configData: Partial<CurrentConfig>, callback?: fs.NoParamCallback) => {
        ...
        const tmpFilePath = `${filePath}.${uuid()}.tmp`;

        if (callback) {
            fs.writeFile(tmpFilePath, json, 'utf8', (err) => {
                if (err) {
                    log.error(`Failed to write config to ${tmpFilePath}: ${err}`);
                    callback(err);
                    return;
                }
                ...
                fs.rename(tmpFilePath, filePath, (renameErr) => {
                    if (renameErr) {
                        log.error(`Failed to rename ${tmpFilePath} to ${filePath}: ${renameErr}`);
                        try {
                        ...
                        log.verbose(`Successfully saved config to ${filePath}`);
                    }
                    callback(renameErr);
                });
            });
            ...
            try {
                ...
                log.verbose(`Successfully saved config to ${filePath} (sync)`);
            } catch (err) {
                log.error(`Failed to write config to ${filePath} (sync): ${err}`);
                try {
                    fs.unlinkSync(tmpFilePath);
                } catch (e) {
                    // ignore
