    ...
    init = () => {
        ...
        Config.predefinedServers.forEach((server) => {
            try {
                ...
                log.error('Failed to create predefined server', server.name, e);
            }
        });

        // Always load local servers if they exist, even if management is disabled,
        // to prevent users from being locked out of their existing sessions.
        const localServers = Config.localServers || [];
        if (localServers.length > 0) {
            initialServers.push(...localServers.sort((a, b) => a.order - b.order).map((server) => {
                try {
                    return this.createServer(server, false);
                } catch (e) {
                    log.error('Failed to create local server', server.name, e);
                    return null;
                }
            }).filter((s): s is MattermostServer => s !== null));
        }
