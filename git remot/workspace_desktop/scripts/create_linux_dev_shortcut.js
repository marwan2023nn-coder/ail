...
if (process.platform === 'linux') {
    ...
    if (fs.existsSync(xdgDir) && !fs.existsSync(path.resolve(xdgDir, 'workspace-desktop-dev.desktop'))) {
        fs.writeFileSync(
            path.resolve(xdgDir, 'workspace-desktop-dev.desktop'),
            `[Desktop Entry]
Name=Mattermost.Dev
Exec=${path.resolve(process.cwd(), 'node_modules/electron/dist/electron')} ${path.resolve(process.cwd(), 'dist')} %U
Terminal=false
Type=Application
Icon=workspace-desktop
...
`,
        ...
        fs.appendFileSync(defaultsListPath, 'x-scheme-handler/mattermost-dev=workspace-desktop-dev.desktop\n');
        ...
        fs.appendFileSync(mimeCachePath, 'x-scheme-handler/mattermost-dev=workspace-desktop-dev.desktop\n');
